...
async function notifyTelegram(booking) {
    ...
    const { data: bookingServices, error } = await supabaseClient_1.supabase
    ...
    // ✍️ Formatierte Namen
    const serviceNames = bookingServices.map(bs => serviceNameMap[bs.service_id] || bs.service_id);
...
`.trim();
    ...
    const response = await (0, node_fetch_1.default)(`https://api.telegram.org/bot${BOT_TOKEN}/sendMessage`, {
        method: 'POST',
